...
$("#dependency-counts-graph").each(function () {
  ...
  David.renderDependencyCountsGraph = function (data) {
    ...
    function transformData (data) {
      return {
        children: Object.keys(data).map(function (depName) {
          return {depName: depName, value: data[depName]}
        })
      }
    }
    ...
    var nodeEnter = nodes.enter()
      .append("g")
      .attr("class", "node")
      .attr("transform", function () { return "translate("+(diameter/2)+","+(diameter/2)+")" })
      .on("click", function (d) { window.location = "http://npmjs.org/package/" + d.depName })
    ...
    var nodeUpdate = nodes.transition().attr("transform", function (d) { return "translate("+d.x+","+d.y+")" })
