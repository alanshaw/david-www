  ...
  var tasks = modVers.map((ver) => {
    return (cb) => {
      npm.commands.view([modName, 'time'], true, (err, data) => {
        ...
        var versionsByDate = Object.keys(time).filter((ver) => {
        ...
        var ascPublishDates = Object.keys(time).map((version) => {
          return time[version]
        }).sort()
        ...
        getUserRepo(modName, githubConfig.host, (err, user, repo) => {
          ...
          getPublishDates(modName, [fromVer, toVer], (err, dates) => {
            ...
            gh.issues.repoIssues(issuesOpts, (err, issues) => {
              ...
              var commitsOpts = {user, repo, since: dates[0], until: dates[1]}
