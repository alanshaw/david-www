...
module.exports = function (db, registry, nsp, brainsConfig, npmConfig) {
  ...
  var Brains = {
     ...
     */
    getInfo: function (manifest, opts, cb) {
      ...
      getDependencies(db, manifest, davidOptions, brainsConfig, function (err, deps) {
        if (err) return cb(err)

        // Get ALL updated dependencies including unstable
        getUpdatedDependencies(db, manifest, davidOptions, brainsConfig, function (err, updatedDeps) {
          if (err) return cb(err)

          davidOptions.stable = true

          // Get STABLE updated dependencies
          getUpdatedDependencies(db, manifest, davidOptions, brainsConfig, function (err, updatedStableDeps) {
            if (err) return cb(err)
...
function getCachedDependencies (db, manifest, opts, cb) {
  var pkgs = {}
...
function getDependencies (db, manifest, opts, brainsConfig, cb) {
  // Get the dependency info we already have cached information for
  getCachedDependencies(db, manifest, opts, function (err, cachedInfos) {
    if (err) return cb(err)
...
function getUpdatedDependencies (db, manifest, opts, cacheTime, cb) {
  getDependencies(db, manifest, opts, cacheTime, function (err, infos) {
    if (err) return cb(err)
