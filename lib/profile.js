...
module.exports = function (manifest, brains, github) {
  var Profile = {
    /**
     ...
     */
    get: function (user, authToken, cb) {
      getRepos(github, user, authToken, function (err, repos) {
        if (err) return cb(err)

        // Get repository status information
        async.parallel(
          repos.map(function (repo) {
            return createGetInfoTask(manifest, brains, user, repo, authToken)
          }),
          function (err, data) {
            if (err) return cb(err)
 ...
 */
function getRepos (github, user, authToken, options, cb) {
 ...
 * @returns {Function}
 */
function createGetInfoTask (manifest, brains, user, repo, authToken) {
  return function (cb) {
