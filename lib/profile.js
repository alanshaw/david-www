 ...
 * @returns {Function}
 */
function createGetInfoTask (user, repo, path, authToken) {
  return function (cb) {
    manifest.getManifest(user, repo.name, path, null, authToken, function (er, manifest) {
 ...
 */
module.exports.get = function (user, authToken, path, cb) {
  getRepos(user, authToken, function (er, repos) {
    ...
    async.parallel(
      repos.map(function (repo) {
        return createGetInfoTask(user, repo, path, authToken)
      }),
      function (er, data) {
        if (er) return cb(er)
